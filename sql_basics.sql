...
select sum(payment.amount) as "Totla paid by each cumtomer",customer.first_name,customer.last_name from payment 
...
select a.title,count(c.rental_id) as "number of time rented" from film a,inventory b, rental c
...
select s.store_id,sum(p.amount) from store s,payment p,rental r,inventory i 
...
select c.name as "Genres",sum(p.amount) as "Gross Revenue"  from film_category f, category c,payment p,inventory i,rental r
where f.category_id=c.category_id and f.film_id=i.film_id and i.inventory_id=r.inventory_id 
and r.rental_id=p.rental_id group by f.category_id order by sum(p.amount) desc limit 5 ;
...
create view TopGenresByGrossRevenue as select c.name as "Genres",sum(p.amount) as "Gross Revenue"  from film_category f, category c,payment p,inventory i,rental r
where f.category_id=c.category_id and f.film_id=i.film_id and i.inventory_id=r.inventory_id 
and r.rental_id=p.rental_id group by f.category_id order by sum(p.amount) desc limit 5 ;
